...
class Game {
  ...
  async create() {
    ...
    try {
      ...
      this.createBarsAndLabels(font, barMaterial);
  ...
  createBarsAndLabels(font, barMaterial) {
    let x = X_BUFFER;
  ...
  updateAfterGui() {
    ...
    this.bars.forEach((bar, index) => {
      ...
      bar.scale.set(
        this.barWidthScalingFactor,
        this.scalingFactor,
        1 // z-scale is unaffected
      );
      bar.position.set(
        x * this.horizontalScalingFactor,
        (growth * this.scalingFactor) / 2,
        0
      );
    });
    ...
    this.textLabels.forEach((text, index) => {
      ...
      const textWidth = (text.geometry.boundingBox.max.x - text.geometry.boundingBox.min.x) * this.textSizeScalingFactor;
