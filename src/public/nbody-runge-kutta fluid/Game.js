...
class Game {
  ...
  handleCylinderCollision() {
    const restitution = 0.8; // adjust this value between 0 and 1
    for (let i = 0; i < this.numParticles; i++) {
      ...
      if (radialDistance + 0.05 > this.cylinderRadius) {
        const correctionFactor = this.cylinderRadius - radialDistance + 0.05;
        ...
        velocity.sub(vAlongNormal).add(vAlongNormal.negate().multiplyScalar(restitution));
