...
class Game {
  constructor(scene, numParticles = 1000) {
    this.scene = scene;
    ...
    this.addSphericalContainer(5); // Example radius, adjust as needed
  ...
  handleBoundaryConditions() {
    const containerRadius = 5; // Ensure this matches the radius used to create the visual container
    for (let i = 0; i < this.numParticles; i++) {
      ...
      if (pos.length() > containerRadius) {
        // Move particle back inside the sphere by reflecting its position about the sphere's surface
        pos.setLength(containerRadius);
  ...
  computeForces() {
    const viscosityCoefficient =
      45 / (Math.PI * Math.pow(this.searchRadius, 6));
    const gravity = new THREE.Vector3(0, -0.981, 0); // Gravity force
    const mass = 1; // Assume each particle has a mass of 1 for simplicity

    for (let i = 0; i < this.numParticles; i++) {
      let forcePressure = new THREE.Vector3(0, 0, 0);
      let forceViscosity = new THREE.Vector3(0, 0, 0);
      ...
      for (const j of this.neighbors[i]) {
        if (i !== j) {
          ...
          const velocityI = new THREE.Vector3(
            this.velocities[i * 3],
            this.velocities[i * 3 + 1],
            this.velocities[i * 3 + 2]
          );
          const velocityJ = new THREE.Vector3(
            this.velocities[j * 3],
          ...
          const distanceVec = new THREE.Vector3()
            .subVectors(posI, posJ)
            .normalize();

          // Pressure force calculation
          let pressureForceMagnitude =
            -mass *
            (this.pressures[i] / (this.densities[i] * this.densities[i]) +
              this.pressures[j] / (this.densities[j] * this.densities[j]));
          let forceP = distanceVec.multiplyScalar(
            pressureForceMagnitude * Math.pow(this.searchRadius - distance, 2)
          );
          forcePressure.add(forceP);

          // Viscosity force calculation
          let viscosityForce = new THREE.Vector3()
            .subVectors(velocityJ, velocityI)
            .multiplyScalar(
          ...
          forceViscosity.add(viscosityForce);
        }
      }

      // Sum forces and apply gravity
      let totalForce = new THREE.Vector3()
        .addVectors(forcePressure, forceViscosity)
        .add(gravity.multiplyScalar(mass));

      // Ensure densities are valid to avoid NaN in acceleration calculation
      const densityI = Math.max(this.densities[i], 1e-6);

      this.accelerations[i * 3] = totalForce.x / densityI;
      this.accelerations[i * 3 + 1] = totalForce.y / densityI;
      this.accelerations[i * 3 + 2] = totalForce.z / densityI;
    }
  }
  integrate(dt) {
    for (let i = 0; i < this.numParticles; i++) {
  ...
  update() {
    ...
    let dt = 0.001; // Example, adjust based on your simulation needs
