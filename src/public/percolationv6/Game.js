...
class Game {
  ...
  generateEdges(vertices) {
    ...
    for (const { from: fromIdx, to: toIdx } of this.lug2.getEdgeList()) {
      edges[edgeIdx++] = vertices[fromIdx * 3];
      edges[edgeIdx++] = vertices[fromIdx * 3 + 1];
      edges[edgeIdx++] = vertices[fromIdx * 3 + 2];
      edges[edgeIdx++] = vertices[toIdx * 3];
      edges[edgeIdx++] = vertices[toIdx * 3 + 1];
      edges[edgeIdx++] = vertices[toIdx * 3 + 2];
    }
    return edges;
  }
